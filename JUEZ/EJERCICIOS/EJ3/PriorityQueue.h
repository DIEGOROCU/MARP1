...
private:
   ...
   void pop2() {
      array[2] = std::move(array.back());
      array.pop_back();
      if (size() >= 2) hundir(2);
   }

   int sumarPrioridades() {
      using value_type = T;
      value_type acumulado = 0;
      PriorityQueue<T, Comparator> copia = *this;
      // repetir hasta que quede como máximo un elemento
      while (copia.size() > 1) {
         value_type a = copia.top(); copia.pop();
         value_type b = copia.top(); copia.pop();
         acumulado += a + b;
         copia.push(a + b);
      }
